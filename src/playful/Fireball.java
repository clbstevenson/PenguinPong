...
public class Fireball implements Entity {
	...
	public Fireball(String type, int health, Animation animation,
			Vector2f startPosition, Vector2f startVelocity,
			Vector2f startAcceleration) throws SlickException {
		this.type = type;
		this.health = health;
		...
		try {
			...
			throw new SlickException(
					"FIREBALL - Failed to load particle system", e);
		}

		missedCannonballSound = new Sound("res/missedCannonball.wav");
	}

	public Shape getShape() {
		if (getType().equals("Boss Fire")) {
			return new Circle(this.pos.x + 19.0F * this.scale, this.pos.y
					+ 19.0F * this.scale, 20.0F * this.scale);
		}
		float[] points = { this.pos.x + 2.0F * this.scale,
				this.pos.y - 1.0F * this.scale, this.pos.x + 8.0F * this.scale,
				this.pos.y - 1.0F * this.scale,
				this.pos.x + 11.0F * this.scale,
				this.pos.y + 3.0F * this.scale,
				this.pos.x + 11.0F * this.scale,
				this.pos.y + 10.0F * this.scale,
				this.pos.x + 7.0F * this.scale,
				this.pos.y + 21.0F * this.scale,
				this.pos.x + 4.0F * this.scale,
				this.pos.y + 21.0F * this.scale, this.pos.x + 0.0F,
				this.pos.y + 10.0F * this.scale, this.pos.x + 0.0F,
				this.pos.y + 3.0F * this.scale };
		...
		if (this.active) {
			...
			if (!this.fell) {
				...
				if (this.pos.x + this.animation.getWidth() >= gc.getWidth()) {
					this.vel.x *= -1.0F;
					while (this.pos.x + this.animation.getWidth() >= gc
							.getWidth()) {
						this.pos.x -= 1.0F;
					}
				}
